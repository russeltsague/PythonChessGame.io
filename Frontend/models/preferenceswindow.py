...
from chess.Backend.controller import configurations
...
class PreferencesWindow():
	...
	def create_preferences_window(self):
		...
		self.pref_window.title("Set Board Colors")
		self.create_preferences_list()
		self.pref_window.transient(self.parent)

	def create_preferences_list(self):
		Label(self.pref_window, text="Board Color 1").grid(row=1, sticky=W, padx=5, pady=5)
		Label(self.pref_window, text="Board Color 2").grid(row=2, sticky=W, padx=5, pady=5)
		Label(self.pref_window, text="Highlight Color").grid(row=3, sticky=W, padx=5, pady=5)
		self.board_color_1_button = Button(self.pref_window, text='Select Board Color 1', command=self.set_color_1)
		self.board_color_1_button.grid(row=1, column=1, columnspan=2, sticky=E, padx=5, pady=5)
		self.board_color_2_button = Button(self.pref_window, text='Select Board Color 1', command=self.set_color_2)
		self.board_color_2_button.grid(row=2, column=1, columnspan=2, sticky=E, padx=5, pady=5)
		self.highlight_color_button = Button(self.pref_window, text='Select Highlight color', command=self.set_highlight_color)
		self.highlight_color_button.grid(row=3, column=1, columnspan=2, sticky=E, padx=5, pady=5)
		Button(self.pref_window, text="Save", command=self.on_save_button_clicked).grid(row=4, column=2, sticky=E, padx=5, pady=5)
		Button(self.pref_window, text="Cancel", command=self.on_cancel_button_clicked).grid(row=4, column=1, sticky=E, padx=5, pady=5)
	...
	def set_new_values(self):
		...
		with open('color_options.ini', 'w') as config_file:
